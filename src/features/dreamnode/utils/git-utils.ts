...
export async function openInTerminalWithPrompt(repoPath: string, prompt: string): Promise<void> {
  try {
    ...
    return runBashInNewTerminalTab(bashCommand);
  } catch (error) {
...
/**
 * Open a new Terminal tab and run a raw bash command.
 * The caller is responsible for all escaping within the bash command itself.
 * This function only handles the AppleScript escaping layer.
 */
export async function runBashInNewTerminalTab(bashCommand: string): Promise<void> {
